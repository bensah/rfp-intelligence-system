...
st.markdown(
    ...
    f"margin:0.15rem 0 0.5rem;'>Year-to-date Summary ({year})</h2>",
...
# Row 2 — role mix (centered, slightly lower; 3 cards across the middle). Each card reads
# "<proceed-in-this-role> of <total unique> (<pct>%)" — a SINGLE denominator (all unique
# RFPs) so the three roles are directly comparable, e.g. "17 of 75 (23%)". Not per-role
# subsets (which read as unrelated fractions like 17-of-45 / 8-of-22).
...
_total_unique = len(unique)


def _role_val(n: int) -> str:
    pct = (n / _total_unique * 100) if _total_unique else 0.0
...
with rc1:
    _kpi("Prime Opportunities", _role_val(proc_prime), "Proceed as prime applicant")
with rc2:
    _kpi("Sub Opportunities", _role_val(proc_sub), "Proceed, applying as Sub")
with rc3:
    _kpi("TA Provider", _role_val(proc_ta), "Technical assistance only")
