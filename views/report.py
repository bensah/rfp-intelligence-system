...
#   "Yauba"           → should be "Yauba Saidu"
#   "Ben"             → should be "Bernard Nsah"  (nickname)
#   "BERNARD NSAH"    → should be "Bernard Nsah"  (case)
#   "bernard nsah"    → should be "Bernard Nsah"
...
#      canonical name. Subset rule handles "Yauba" → "Yauba Saidu".
#   4. Tie-break: longest canonical wins (so "Bernard Nsah" beats "Bernard").
...
# bite us later if a Yauba Issa joins alongside Yauba Saidu.
# ---------------------------------------------------------------------------
_NICKNAME_TO_FULL = {
...
def normalize_member_name(raw: str | None) -> str:
    ...
    for c in canonical_list:
        ...
        if not c_tokens:
            continue
        # Input ⊆ canonical (e.g. "Yauba" ⊆ "Yauba Saidu") OR
        # canonical ⊆ input (rare — e.g. "Bernard Nsah Cameroon" submitted)
        if input_tokens <= c_tokens or c_tokens <= input_tokens:
            matches.append(c)
    if matches:
        # Tie-break: prefer the canonical name with MORE tokens (the
        # fully-specified form). "Bernard Nsah" beats "Bernard".
...
st.markdown(
    """
    <style>
      /* Print-only rules so window.print() yields a clean dashboard. */
      @media print {
...
period_mode = pc1.selectbox(
    "Period",
    options=["This year", "Year to date", "Specific month",
             "Last 90 days", "Last 12 months", "All time"],
    index=1,  # YTD is the most useful default per user request
...
bucket_mode = pc4.selectbox(
    "View by",
    options=["Weekly", "Monthly", "Quarterly", "Semestrial", "Annually"],
    index=1,
...
if rfps_all.empty:
    ...
    if "progress_status" in unique_all.columns:
        ps = (
...
if not engagements.empty and engagements["engagement_date"].notna().any():
...
if not engagements.empty and "donor" in engagements.columns:
    top_donors = (
...
if rfps_all.empty:
    ...
    if sub_period.empty or "submitted_by" not in sub_period.columns:
        ...
        by_member = (
            sub_period.assign(
                # Normalize names so "Ben" + "Bernard Nsah" + "BERNARD NSAH"
                # all collapse onto the canonical "Bernard Nsah" series.
    ...
    for col_name, col_label, container in [
        ...
        if col_name in activity_rows.columns:
            counts = (
...
if rfps_all.empty:
    ...
    if not approved_period.empty and "amount_secured" in approved_period.columns:
    ...
    if {"amount_requested", "amount_secured"} <= set(unique.columns):
