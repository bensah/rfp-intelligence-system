...
-- "the organisation Cameroon", "the organisation Programme Team") and scopes every PER-TENANT data
-- table to a tenant_id, backfilling all existing rows to the the organisation Cameroon tenant.
--
...
-- 3. SEED the the organisation Cameroon tenant, carrying the existing single org_profile into it.
--    (app_settings is key/value with a TEXT value → cast to jsonb; missing → '{}'.)
insert into tenants (name, slug, status, org_profile)
values (
  'the organisation Cameroon', 'sample-country-team', 'active',
  coalesce((select value from app_settings where key = 'org_profile'), '{}')::jsonb
)
on conflict (name) do nothing;

-- 3b. Make every EXISTING user an ACTIVE member of the organisation Cameroon, preserving their role.
...
from users u
cross join (select id from tenants where name = 'the organisation Cameroon') t
on conflict (tenant_id, user_id) do nothing;

-- 4. Add tenant_id to each PER-TENANT table, backfill existing rows to the organisation Cameroon,
...
begin
  select id into _tid from tenants where name = 'the organisation Cameroon';
  if _tid is null then
    raise exception 'the organisation Cameroon tenant seed failed — aborting';
  end if;
