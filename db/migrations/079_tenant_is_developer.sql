...
--                   and "Taadom Digital PLC"). Members of a developer tenant may
...
-- cover MORE than one tenant (RFPIS Inc + Taadom), so it needs its own flag. The
...
-- platform → developer seed. Flag the SECOND developer tenant (Taadom) from the UI
...
-- 2. Best-effort: also flag a tenant literally named like "Taadom" if one
...
update tenants set is_developer = true
 where is_developer = false
   and (name ilike '%taadom%' or slug ilike '%taadom%'
