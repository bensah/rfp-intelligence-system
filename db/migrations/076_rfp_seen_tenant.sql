...
-- backfilled to CHAI Cameroon (they were CHAI's historically). Single-tenant/super_user
...
-- Backfill the pre-existing global tombstones to CHAI Cameroon (their historical owner).
update rfp_seen
   set tenant_id = (select id from tenants where name = 'CHAI Cameroon' limit 1)
 where tenant_id is null
   and exists (select 1 from tenants where name = 'CHAI Cameroon');
