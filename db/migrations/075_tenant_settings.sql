...
-- (Taadom) still saw CHAI Cameroon's schedule, and why every tenant would screen against
-- CHAI's countries/themes.
--
...
-- — NOT to CHAI's config. CHAI Cameroon's CURRENT values are seeded below so it keeps
...
-- Seed CHAI Cameroon's current global schedule / policies / team into its own
-- tenant_settings so nothing changes for CHAI. Fresh tenants get code defaults.
