...
--      their sole home — comment this out to keep the super_user in CHAI Cameroon too;
--   D. SEEDS CHAI Cameroon's CURRENT global identity (from app_settings) into its own
--      org_identity, so CHAI users + the scan see the SAME identity/flags as today once
...
begin
  ...
  if _uid is null then
    ...
    -- C. OPTIONAL — make RFPIS Inc. the super_user's ONLY tenant (drops CHAI Cameroon
  ...
  -- D. Seed CHAI Cameroon's CURRENT global identity into its own org_identity, so a
  --    CHAI user (and in-session scoring) sees the same values as today. RFPIS Inc.
  --    stays blank. Only runs if CHAI Cameroon exists and its org_identity is still {}.
  update tenants t
...
--   begin;
--     -- (re-add the super_user to CHAI Cameroon if step C removed them, if desired)
