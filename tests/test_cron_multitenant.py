...
class MultiTenantDetectionTests(unittest.TestCase):
    ...
    def test_jwt_off_two_non_platform_is_multitenant(self):
        self._set_jwt(False)
        tenants = [_t("RFPIS APP", platform=True), _t("CHAI"), _t("Taadom")]
    ...
    def test_jwt_off_single_non_platform_is_single_tenant(self):
        self._set_jwt(False)
        self.assertFalse(SP.is_multitenant_deploy([_t("RFPIS APP", platform=True), _t("CHAI")]))
        self.assertFalse(SP.is_multitenant_deploy([_t("CHAI")]))
