...
landed in the alphabetically-first NON-platform tenant (the "lands in CHAI Cameroon" bug).
...
# Two non-platform memberships, alphabetical first = "CHAI Cameroon".
_CHAI = {"tenant_id": "28b17088", "name": "CHAI Cameroon", "slug": "chai-cameroon",
         "role": "admin", "is_platform": False}
_TAADOM = {"tenant_id": "df584f3e", "name": "Taadom Digital PLC", "slug": "taadom",
           "role": "admin", "is_platform": False}
...
class LandingSafeguardTests(unittest.TestCase):
    def test_super_multi_no_platform_resolves_directly(self):
        ...
        # NOT the alphabetically-first CHAI Cameroon.
        with mock.patch.object(tc, "_platform_home_membership", return_value=_PLATFORM_DIRECT):
            chosen = tc._default_membership(_SUPER, [_CHAI, _TAADOM])
    ...
    def test_super_multi_with_platform_membership_used_without_direct_lookup(self):
        ...
        with mock.patch.object(tc, "_platform_home_membership",
            ...
            chosen = tc._default_membership(_SUPER, [_CHAI, _PLATFORM_MEM, _TAADOM])
        self.assertEqual(chosen["tenant_id"], "13d79b44")
    ...
    def test_super_multi_direct_none_falls_back_to_alphabetical(self):
        ...
        with mock.patch.object(tc, "_platform_home_membership", return_value=None):
            chosen = tc._default_membership(_SUPER, [_TAADOM, _CHAI])
        self.assertEqual(chosen["name"], "CHAI Cameroon")   # alphabetical first
    ...
    def test_single_membership_unchanged(self):
        ...
        with mock.patch.object(tc, "_platform_home_membership",
                               side_effect=AssertionError("should not be called")):
            self.assertEqual(tc._default_membership(_SUPER, [_TAADOM])["tenant_id"], "df584f3e")
            self.assertEqual(tc._default_membership(_USER, [_CHAI])["tenant_id"], "28b17088")
    ...
    def test_nonsuper_multi_prefers_remembered_then_alphabetical(self):
        ...
        with mock.patch.object(tc, "_platform_home_membership",
            ...
            first = tc._default_membership(_USER, [_TAADOM, _CHAI])
            self.assertEqual(first["name"], "CHAI Cameroon")
