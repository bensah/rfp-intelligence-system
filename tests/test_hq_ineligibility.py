...
are not eligible"). MUST-1 item C must:
  * DISQUALIFY an org whose HQ is outside the required region (Example Country Team, HQ = United
...
class HqIneligibilityTests(unittest.TestCase):
    def test_hq_outside_region_is_declined(self):
        # Example Country Team: HQ = United States → excluded from a Sub-Saharan-Africa-HQ call.
