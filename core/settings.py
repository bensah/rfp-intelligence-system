...
# at CHAI BDT (the reference deployment) so a fresh install isn't blank.
# -----------------------------------------------------------------------------
_ORG_DEFAULTS = {
    "org_name":            "CHAI Cameroon — Business Development Team",
    "org_short":           "CHAI BDT",
...
def get_org() -> dict[str, str]:
    ...
    the CHAI BDT defaults so the UI never renders an empty header."""
    out: dict[str, str] = {}
...
def get_org_short() -> str:
    """Short name for page titles / breadcrumbs (e.g. 'CHAI BDT')."""
