...
def synthesize(candidate: dict[str, Any], org: dict[str, Any],
    ...
    user = (
        ...
        "indirect_cost_max_pct (a NUMBER 0-100 — the maximum indirect / overhead / "
        "administrative rate the call allows, as a % of total project cost, e.g. 15 for "
        "'indirect costs up to a maximum of 15% of the total budget'; OMIT if the call "
        "states no such cap), "
        ...
        "hq_country_required (the country OR REGION the applicant/lead institution must be "
        "HEADQUARTERED in, verbatim — e.g. a country like 'Kenya' OR a region like "
        "'Sub-Saharan Africa'; set ONLY when the call restricts where the applicant is "
        "headquartered/registered/incorporated, e.g. 'organizations headquartered outside "
        "sub-Saharan Africa are not eligible' → 'Sub-Saharan Africa'); "
