# the organisation Cameroon — RFP Intelligence System
## Complete Build Plan for Claude Code
**Version:** 1.0 | **Date:** June 2026 | **Owner:** Jane Doe (bensah @ GitHub)

---

## 1. Project Overview

The RFP Intelligence System (RFPIS) is a Streamlit-based web application that replaces the Excel-based the organisation Cameroon RFP Eligibility Screener. It automates weekly RFP discovery, scores opportunities against the organisation's nine eligibility criteria, enables manual submission by collaborators, and reproduces all existing dashboard views in a live, access-controlled web interface.
...
- Access-controlled: the organisation Cameroon team only
...
must_3_implementable:     0.15   # High — the organisation execution advantage
...
Form sections:
1. **Opportunity Description:** Title, Funder, Geography (multi-select), Program Area (multi-select), Link, Brief Description, Date Posted, the organisation Role, Window, Deadline, Award Date, Value, Currency, Duration
...
- Subject: `the organisation Cameroon | RFP Digest — Week [N] | [N] new opportunities`
- Body (HTML table):
  - New RFPs found this week, ranked by alignment_score descending
  - Columns: Title, Funder, Geography, Deadline, Value, the organisation Role, Alignment Score, URL
...
Claude Code tasks:
1. Initialise repository at `bensah/rfp-intelligence` with full folder structure
...
```bash
# Clone repository
git clone https://github.com/bensah/rfp-intelligence.git
cd rfp-intelligence
...
1. Push repository to `github.com/bensah/rfp-intelligence`
2. Go to `share.streamlit.io` → New app → Connect GitHub → select `bensah/rfp-intelligence`
3. Set main file path to `app.py`
4. Add all secrets under "Advanced settings → Secrets"
5. Click Deploy — Streamlit assigns a URL (customisable to `rfp.streamlit.app`)
...
> "Create the full folder structure for `rfp-intelligence` as described in the build plan. Generate `requirements.txt`, `.streamlit/config.toml` with the organisation green branding (`#00703C` primary), and a placeholder `app.py` that requires login before showing any page."
...
| Hosting | Streamlit Community Cloud (bensah) | Free; the organisation-branded; public URL |
...
*the organisation Cameroon — Business Development Team | Prepared for build with Claude Code*