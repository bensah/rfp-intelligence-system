...
def map_form1_row_by_header(row: list[Any], col_map: dict[str, int],
    ...
    return {
        ...
        # `applicant_role` is the legacy column name from the original Excel
        ...
        # spirit (applicant role) but kept under applicant_role for back-compat.
        "applicant_role": _txt(get("the organisation Role", "Applicant Role", "Role")),
...
def map_engagement_row(row: list[Any]) -> Optional[dict[str, Any]]:
    ...
    if not edate:
        return None
    donor = _txt(c(3))
    application_lead = _txt(c(6))
    ext_key = f"{edate}|{(donor or '').strip().lower()}|{(application_lead or '').strip().lower()}"
    return {
        "engagement_date": edate,
        "donor": donor,
        "engagement_type": _txt(c(4)),
        "format": _txt(c(5)),
        "application_lead": application_lead,
...
def migrate(xlsx_path: Path, dry_run: bool = False) -> None:
    ...
    merge_by_external_id(
        ...
        updatable_fields=[
            ...
            "application_lead", "donor_contacts", "purpose", "outcome", "linked_rfp_uid",
        ],
    )
